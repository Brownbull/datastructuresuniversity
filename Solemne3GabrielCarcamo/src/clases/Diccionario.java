...
public class Diccionario<T> {
    ...
    private boolean existe(Nodo<T> p, Rut clave){
        if (p==null)
            ...
            if (clave.compareTo(p.getDato().getKey()) == 0)
                return true;
           else
                if (clave.compareTo(p.getDato().getKey()) < 0)
    ...
    private void mostrar(Nodo<T> p, char dv){
        if (p!=null){
            if (p.getDato().getKey().getdVerificador() == dv){

                System.out.println(p.getDato().getKey());
            }
            mostrar(p.getIzq(), dv);
            mostrar(p.getDer(),dv);
        }
    ...
    private Nodo<T> insertar(Nodo<T> p, Base<T> e){
        if (p==null){
            p= new Nodo<>(e);
            System.out.println("Creado.. " + e.getKey().toString());
        }
        else{
            if (e.getKey().compareTo(p.getDato().getKey()) < 0){
                System.out.println("\tHijo izq de " + p.getDato().getKey().toString());
                p.setIzq(insertar(p.getIzq(), e));
            }
            else{
                if (e.getKey().compareTo(p.getDato().getKey()) > 0){
                    System.out.println("\tHijo der de " + p.getDato().getKey().toString());
